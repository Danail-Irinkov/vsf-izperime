...
export default {
  ...
  methods: {
    ...
    invaildCard () {
      ...
      } else if (!this.formData.cardCvv || this.formData.cardCvv.length !== 3) {
		    alert(this.$t('cardForm.invalidCardCvv'))
      } else if (!this.formData.cardMonth || !this.formData.cardYear) {
		    alert(this.$t('cardForm.invalidCardExpiration'))
      } else if (!this.formData.cardName) {
		    alert(this.$t('cardForm.invalidCardName'))
      } else {
      	this.formData.CardType = this.$refs.card.CardType
	      this.$emit('submit')
      }
    },
    blurCardNumber () {
...
<style lang="scss">
	@import "assets/scss/cardstyles.scss";
</style>
